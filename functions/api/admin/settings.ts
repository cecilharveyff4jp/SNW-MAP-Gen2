...
export const onRequestPut: PagesFunction<AdminEnv> = async (context) => {
  ...
  try {
    if (typeof body.serverNo === "string") await upsert(KEYS.server, body.serverNo.trim().slice(0, 32));
    if (typeof body.allianceName === "string") await upsert(KEYS.name, body.allianceName.trim().slice(0, 64));
