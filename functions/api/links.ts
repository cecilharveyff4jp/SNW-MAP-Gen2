...
interface Row { id: number; label: string; url: string; sort_order: number }

export const onRequestGet: PagesFunction<Env> = async (context) => {
  try {
    const { results } = await context.env.DB.prepare(
      "SELECT id, label, url, sort_order FROM links ORDER BY sort_order, id"
    ).all<Row>();
    const links = (results ?? []).map((r) => ({ id: r.id, label: r.label, url: r.url, sortOrder: r.sort_order }));
