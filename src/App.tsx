...
export default function App() {
  ...
  const routed = path === "/account" ? (<CenteredPage><AccountPanel me={me} onReload={loadMe} /></CenteredPage>)
    ...
    : path === "/survey" ? (<CenteredPage><SurveyPage canEdit={canEdit} /></CenteredPage>)
...
function MapView({ canEdit, isOwner, me, alliance, newsUnread = 0 }: { canEdit: boolean; isOwner: boolean; me: Me | null; alliance: AllianceInfo | null; newsUnread?: number }) {
  ...
  const [heatmap, setHeatmap] = useState(() => { try { return localStorage.getItem("snw_heatmap") === "1"; } catch { return false; } });
  const toggleHeatmap = () => setHeatmap((v) => { const nv = !v; try { localStorage.setItem("snw_heatmap", nv ? "1" : "0"); } catch { /* noop */ } return nv; });
  ...
  const toggleMapVisible = async (id: number) => {
    const cur = maps.find((m) => m.id === id); if (!cur || cur.isBase) return;
    try { await updateMap(id, { isVisible: !cur.isVisible }); await loadMaps(); setToast(cur.isVisible ? "このマップを非表示にしました（メンバーに出ません）" : "このマップを表示にしました"); }
    catch (e) { dlg.alert({ title: "エラー", message: String((e as Error).message || e) }); }
  };
    ...
    <div style={{ flex: 1, minHeight: 0, display: "flex", flexDirection: "column" }}>
      ...
      <div style={{ flex: 1, minHeight: 0, position: "relative", overflow: "hidden", animation: "snwboot 0.5s ease-out" }}>
        ...
        <div style={{ position: "absolute", top: 12, left: 12, display: "flex", gap: 4, flexWrap: "wrap", alignItems: "center", padding: 6, borderRadius: 13, background: mapDark ? "rgba(18,24,34,0.62)" : "rgba(255,255,255,0.72)", backdropFilter: "blur(12px)", WebkitBackdropFilter: "blur(12px)", border: "1px solid " + (mapDark ? "rgba(255,255,255,0.10)" : "rgba(255,255,255,0.9)"), boxShadow: "0 10px 26px -8px rgba(20,28,54,0.30)" }}>
          ...
          <button onClick={toggleHeatmap} style={{ ...trayBtn(mapDark), background: heatmap ? "var(--accent-soft, #ededfc)" : "transparent", color: heatmap ? "var(--accent-strong, #4b3fc4)" : (mapDark ? "#dfe7f1" : "#33404f") }}><Icon name="chart" size={15} />総力ヒート {heatmap ? "ON" : "OFF"}</button>
        ...
        {heatmap && (
          <div style={{ position: "absolute", left: "50%", bottom: 10, transform: "translateX(-50%)", zIndex: 6, display: "flex", alignItems: "center", gap: 7, padding: "6px 12px", borderRadius: 999, background: mapDark ? "rgba(18,24,34,0.74)" : "rgba(255,255,255,0.92)", border: "1px solid " + (mapDark ? "rgba(255,255,255,0.12)" : "var(--border, #e9edf2)"), boxShadow: "0 2px 10px rgba(15,23,42,0.12)", backdropFilter: "blur(8px)", WebkitBackdropFilter: "blur(8px)", fontSize: 11, fontWeight: 600, color: mapDark ? "#cdd6e3" : "#475063", pointerEvents: "none", maxWidth: "94%", overflowX: "auto", whiteSpace: "nowrap" }}>
            ...
            {([["#4dabf7", "0"], ["#3bc9db", "50M"], ["#51cf66", "150M"], ["#fcc419", "300M"], ["#ff922b", "450M"], ["#fa5252", "600M"]] as [string, string][]).map(([c, l]) => (
