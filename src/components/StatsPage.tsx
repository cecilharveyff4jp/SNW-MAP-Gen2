...
export default function StatsPage({ canEdit }: { canEdit: boolean }) {
  ...
  const fcLevelAt = (x: number, y: number): string | null => {
    // 各レベル行の矩形で当たり判定（タッチでも確実。モックと同方式）。
    let hit: string | null = null;
    document.querySelectorAll<HTMLElement>("[data-fclv]").forEach((el) => {
      const r = el.getBoundingClientRect();
      if (x >= r.left && x <= r.right && y >= r.top && y <= r.bottom) hit = el.getAttribute("data-fclv");
    });
    return hit;
  };
